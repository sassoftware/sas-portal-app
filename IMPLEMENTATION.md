...
To minimize the amount of stored processes to create and to re-use common code, the following stored processes are defined:
...
- deleteItem: This is the entry point used to actually delete the item from metadata (can be called from the page generated by removeItem or called directly (if no additional parameters are reqired).
